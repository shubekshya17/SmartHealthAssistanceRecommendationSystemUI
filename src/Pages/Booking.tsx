...
export default function Booking() {
  ...
  useEffect(() => {
    ...
    else setFilteredDoctors(doctors.filter((d) => d.specialization === selectedSpecialty));
  ...
  useEffect(() => {
    ...
    fetch(
     ...
     // http://localhost:5102/api/Appointment/GetSlots?hospitalId=1&doctorId=1
      ...
      .then(
        (res) => res.json())
        .then((data) => {               // Set the slots and log data
    ...
    // Fetch booked slots
    fetch(`http://localhost:5102/api/Appointment/GetBookedSlots?hospitalId=${selectedHospital}&doctorId=${selectedDoctor}&date=${dateStr}`)
      .then((res) => res.json())
       .then((data) => {               // Set the slots and log data
...
const parseTime = (time12h: string) => {
  ...
  try {
    // BookSlot API
    const res1 = await fetch("http://localhost:5102/api/Appointment/BookSlot", {
      method: "POST",
    ...
    // SaveAppointment API
    const res2 = await fetch("http://localhost:5102/api/Appointment/SaveAppointment", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        doctorId: selectedDoctor,
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f9fafb", padding: "20px" }}>
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>
        ...
        {step === 1 && (
          ...
          >
            ...
            <div style={{ marginBottom: "24px" }}>
              <label style={{ display: "block", marginBottom: "8px" }}>Select Hospital</label>
              <select
            ...
            <div style={{ marginBottom: "16px" }}>
              <label style={{ display: "block", marginBottom: "8px" }}>Filter by Specialty</label>
              <select
              ...
              {filteredDoctors.map((d) => (
                <div
                  ...
                  style={{
                    padding: "16px",
                    borderRadius: "8px",
                    border: selectedDoctor === d.doctorId ? "2px solid #69b3b4" : "2px solid #e5e7eb",
                    backgroundColor: selectedDoctor === d.doctorId ? "#eff6ff" : "white",
                    cursor: "pointer",
                  }}
                >
                  <h3 style={{ fontWeight: 600, margin: 0 }}>{d.fullName}</h3>
                  <p style={{ color: "#6b7280", margin: 0 }}>{d.specialization}</p>
                </div>
              ))}
            </div>
        ...
        {step === 2 && (
          ...
          >
            <h2 style={{ fontSize: "24px", fontWeight: "bold", marginBottom: "8px" }}>Select Date & Time</h2>
            <div style={{ marginBottom: "16px" }}>
...
<div style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginBottom: "24px" }}>
  {getAvailableTimes().map((time) => {
    const isBooked = time.includes("booked-") || bookedSlots.some(
      (b) =>
        b.doctorId === selectedDoctor &&
        b.hospitalId === selectedHospital &&
        new Date(b.date).toISOString().split("T")[0] === selectedDate.toISOString().split("T")[0] &&
    ...
    return (
      <div
        ...
        style={{
          padding: "8px 12px",
          borderRadius: "6px",
          border: selectedTime === displayTime ? "2px solid #69b3b4" : "1px solid #d1d5db",
      ...
      >
        ...
        {step === 3 && (
          ...
          >
            <h2 style={{ fontSize: "24px", fontWeight: "bold", marginBottom: "8px" }}>Patient Details</h2>
            <input
              type="text"
              placeholder="Patient Name"
              value={patientName}
              onChange={(e) => setPatientName(e.target.value)}
              style={{ width: "100%", padding: "8px", marginBottom: "12px", borderRadius: "6px", border: "1px solid #d1d5db" }}
            />
            <input
              type="text"
            ...
            <textarea
              ...
              style={{ width: "100%", padding: "8px", marginBottom: "24px", borderRadius: "6px", border: "1px solid #d1d5db" }}
            />

            <button
              style={{
                width: "100%",
                backgroundColor: patientName && patientPhone && patientEmail ? "#69b3b4" : "#9ca3af",
                color: "white",
                padding: "12px",
                borderRadius: "8px",
                fontWeight: "bold",
                cursor: patientName && patientPhone && patientEmail ? "pointer" : "not-allowed",
              }}
        ...
        {step === 4 && (
          <div style={{
            ...
            <h2 style={{ fontSize: "24px", fontWeight: "bold", margin: "0 0 8px 0" }}>Confirm Appointment</h2>
            <p style={{ color: "#6b7280", margin: "0 0 24px 0" }}>Review your details</p>
            
            <div style={{ backgroundColor: "#f3f4f6", padding: "16px", borderRadius: "8px", marginBottom: "24px" }}>
              <div style={{ marginBottom: "12px" }}>
                <strong>Doctor:</strong> {doctors.find(d => d.doctorId === selectedDoctor)?.fullName}
              </div>
              <div style={{ marginBottom: "12px" }}>
